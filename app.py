...
# 内嵌课程数据作fallback（确保在任何目录运行都能加载课程）
try:
...
def get_user_files(user):
    ...
    user_dir = os.path.join(DATA_DIR, "users", safe_user)
    return {
        "progress": os.path.join(user_dir, "progress.json"),
        "settings": os.path.join(user_dir, "settings.json"),
    }

# 为兼容性保留：如果没有当前用户，使用默认路径
_current_user = get_current_user()
if _current_user:
    _files = get_user_files(_current_user)
    PROGRESS_FILE = _files["progress"]
    SETTINGS_FILE = _files["settings"]
else:
...
def _get_current_files():
    """获取当前用户的文件路径，每次调用时动态计算"""
    user = get_current_user()
    if user:
        return get_user_files(user)
    return {"progress": os.path.join(DATA_DIR, "progress.json"),
            "settings": os.path.join(DATA_DIR, "settings.json")}

def get_progress():
    files = _get_current_files()
    data = load_json(files["progress"], {"checkins": [], "weak_point_stats": {}})
...
def get_settings():
    files = _get_current_files()
    return load_json(files["settings"], {
...
if not current_user:
    ...
    with col1:
        user_input = st.text_input("👤 使用者名称（如：小明、Emma）", "", key="user_input")
    with col2:
        st.write("")
        st.write("")
    ...
    <div style="display: flex; justify-content: flex-end; align-items: center; margin-bottom: 8px;">
        <span style="background-color: #E8F5E9; color: #2E7D32; padding: 4px 12px; border-radius: 12px; font-size: 0.85rem;">
            👤 当前用户：<strong>{current_user}</strong>
        </span>
    </div>
    """, unsafe_allow_html=True)
...
# ==================== 设置 ====================
    ...
    with col1:
        # 导出数据
        files = _get_current_files()
        progress_data = load_json(files["progress"], {"checkins": [], "weak_point_stats": {}})
